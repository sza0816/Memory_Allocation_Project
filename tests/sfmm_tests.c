...
Test(sfmm_student_suite, sf_memalign_invalid_alignment, .timeout = TEST_TIMEOUT) {
	size_t sz_x=sizeof(double), al_x=7;
	void *x=sf_memalign(sz_x,al_x);
...
Test(sfmm_student_suite, sf_memalign_zero_size, .timeout = TEST_TIMEOUT) {
	size_t al_x=sizeof(long);
	void *x=sf_memalign(0, al_x);
...
Test(sfmm_student_suite, sf_memalign_small_size, .timeout = TEST_TIMEOUT) {
	size_t sz_x=sizeof(double), al_x=sizeof(long double)*2;
	void *x=sf_memalign(sz_x, al_x);
	...
	sf_block *bp = (sf_block *)((char *)x - sizeof(sf_header));
	cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp->header & ~0x7) == 40, "Memory aligned block size not what was expected!");
	cr_assert((intptr_t)x % al_x == 0, "Memory does not align!");

	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0,2);
	assert_free_block_count(56, 1);
	assert_free_block_count(3960, 1);
	assert_free_list_size(1, 1);
	assert_free_list_size(7, 1);
}

Test(sfmm_student_suite, sf_memalign_large_size, .timeout = TEST_TIMEOUT ) {
	size_t sz_x=sizeof(long double)*4 + 1, al_x = sizeof(long double)*4;
	void *x=sf_memalign(sz_x, al_x);
	...
	sf_block *bp = (sf_block *)((char *)x - sizeof(sf_header));
	cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp->header & ~0x7) == 72, "Memory aligned block size not what was expected!");
	cr_assert((intptr_t)x % al_x == 0, "Memory does not align!");
...
Test(sfmm_student_suite, sf_memalign_after_sf_malloc, .timeout = TEST_TIMEOUT) {
	size_t sz_x = sizeof(long double)*8 - 1;
	size_t sz_y = sizeof(double)*3 + 1,  al_y = sizeof(long double);
	void *x=sf_malloc(sz_x);
	void *y=sf_memalign(sz_y, al_y);

	cr_assert_not_null(x, "x is NULL!");

	sf_block *bp_x = (sf_block *)((char *)x - sizeof(sf_header));
	cr_assert(bp_x->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");

	sf_block *bp = (sf_block *)((char *)y - sizeof(sf_header));
	cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((intptr_t)y % al_y == 0, "Memory does not align!");
